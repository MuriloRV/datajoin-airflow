"""DAG marelli__conta_azul_etl — pipeline completo Conta Azul.

Pipeline em 3 etapas sequenciais:
    1. extract_entity_to_raw     : Python (mapped task)  -> marelli_raw.conta_azul__*
    2. dbt_staging_curated       : Cosmos                -> marelli_staging + marelli_curated
    3. dbt_delivery              : Cosmos                -> marelli_delivery
...
Modelagem dbt em `dbt/projects/marelli/models/`.
...
def marelli__conta_azul_etl():
    ...
    def extract_entity_to_raw(entity_name: str, **context) -> dict:
        """1) Extrai 1 entidade do Conta Azul e materializa em marelli_raw.
        ...
        with TelemetryContext.from_airflow(context, tenant_id=tenant["id"]) as tele:
            ...
            # Le credenciais OAuth2 da Connection `marelli__conta_azul`.
