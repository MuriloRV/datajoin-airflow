"""DAG atacadao_ferragens__conta_azul_etl — pipeline completo Conta Azul.

Pipeline em 3 etapas sequenciais:
    1. extract_entity_to_raw     : Python (mapped task)  -> atacadao_ferragens_raw.conta_azul__*
    2. dbt_staging_curated       : Cosmos                -> atacadao_ferragens_staging + atacadao_ferragens_curated
    3. dbt_delivery              : Cosmos                -> atacadao_ferragens_delivery
...
Modelagem dbt em `dbt/projects/atacadao_ferragens/models/`.
...
# ─────────────── Configs do tenant ───────────────
TENANT_SLUG = "atacadao_ferragens"
...
@dag(
    ...
    catchup=False,
    max_active_runs=1,
    tags=["tenant:atacadao_ferragens", "kind:etl", "source:conta_azul"],
    doc_md=__doc__,
    on_success_callback=_PIPELINE_FINALIZE_SUCCESS,
    on_failure_callback=_PIPELINE_FINALIZE_FAILED,
)
def atacadao_ferragens__conta_azul_etl():
    ...
    def extract_entity_to_raw(entity_name: str, **context) -> dict:
        """1) Extrai 1 entidade do Conta Azul e materializa em atacadao_ferragens_raw.
        ...
        with TelemetryContext.from_airflow(context, tenant_id=tenant["id"]) as tele:
            ...
            # Le credenciais OAuth2 da Connection `atacadao_ferragens__conta_azul`.
...
atacadao_ferragens__conta_azul_etl()
