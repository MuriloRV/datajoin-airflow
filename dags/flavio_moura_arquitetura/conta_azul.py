"""DAG flavio_moura_arquitetura__conta_azul_etl — pipeline completo Conta Azul.

Pipeline em 3 etapas sequenciais:
    1. extract_entity_to_raw     : Python (mapped task)  -> flavio_moura_arquitetura_raw.conta_azul__*
    2. dbt_staging_curated       : Cosmos                -> flavio_moura_arquitetura_staging + flavio_moura_arquitetura_curated
    3. dbt_delivery              : Cosmos                -> flavio_moura_arquitetura_delivery
...
Modelagem dbt em `dbt/projects/flavio_moura_arquitetura/models/`.
...
# ─────────────── Configs do tenant ───────────────
TENANT_SLUG = "flavio_moura_arquitetura"
...
@dag(
    ...
    catchup=False,
    max_active_runs=1,
    tags=["tenant:flavio_moura_arquitetura", "kind:etl", "source:conta_azul"],
    doc_md=__doc__,
    on_success_callback=_PIPELINE_FINALIZE_SUCCESS,
    on_failure_callback=_PIPELINE_FINALIZE_FAILED,
)
def flavio_moura_arquitetura__conta_azul_etl():
    ...
    def extract_entity_to_raw(entity_name: str, **context) -> dict:
        """1) Extrai 1 entidade do Conta Azul e materializa em flavio_moura_arquitetura_raw.
        ...
        with TelemetryContext.from_airflow(context, tenant_id=tenant["id"]) as tele:
            ...
            # Le credenciais OAuth2 da Connection `flavio_moura_arquitetura__conta_azul`.
...
flavio_moura_arquitetura__conta_azul_etl()
